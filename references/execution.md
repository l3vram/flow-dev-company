...
- **Barrier between waves**: wave N+1 does not start until wave N is green (or explicitly
  marked blocked and skipped by decision).
- A blocked plan does not block its wave siblings. It does block its dependents in later
  waves — those get postponed or escalated to a human gate.

`scripts/flow.sh waves` computes the layering from the state file. Use it. Hand-deriving a
DAG in prose costs expensive output tokens for something that is arithmetic.
...
- The working repo is a git repository. If not, stop and say so — worktree isolation needs it.
- The plan's dependencies show green in the state file.
...
> You are the executor for the plan below. Follow it step by step. Run every verification
...
> Commit in the worktree per the plan's git workflow. SKIP the plan's instruction to update
...
```json
{
  ...
  "budget": { "by_phase": {}, "by_tier": {}, "by_plan": {}, "spawns": 0 }
}
```

- `status`: `pending | running | review | green | blocked`.
- **Circuit breaker**: `attempts >= 3` on a plan → stop retrying, mark blocked, escalate to a
  human gate. Never burn budget on blind retries.
- Update on every transition via `scripts/flow.sh`, not by rewriting JSON in prose.
